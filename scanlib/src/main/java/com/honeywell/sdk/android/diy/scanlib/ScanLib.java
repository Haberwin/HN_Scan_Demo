...
public class ScanLib {
    ...
    private   String action = "com.honeywell.scan";//广播名称
    ...
    //utf-8
    public boolean Encoding_UTF_8=false;
    ...
    private BarcodeReader.BarcodeListener barcodeListener = new BarcodeReader.BarcodeListener() {
        ...
        public void onBarcodeEvent(final BarcodeReadEvent barcodeReadEvent) {
            ...
            if (Encoding_UTF_8)
            {
                ...
                try {
                    byte[] bytes = barcodeReadEvent.getBarcodeData().getBytes("ISO-8859-1");
                    data = new String(bytes, "utf-8");
                }catch (Exception e)
                {e.printStackTrace();}
            }
            else
                data = barcodeReadEvent.getBarcodeData();
            ...
            if (_iScan != null) {
                if (isBroadCastMode) {
                    ...
                    Intent intent1 = new Intent("com.honeywell.ezservice.TRANSMIT_RESULT");
                    intent1.putExtra("transmit_way", 1);
                    ...
                    {e.printStackTrace();}
                    if (addEnterString) {
                        intent1 = new Intent("com.honeywell.ezservice.TRANSMIT_RESULT");
                        intent1.putExtra("transmit_way", 2);
                        intent1.putExtra("data", suffix);
                        appContext.sendBroadcast(intent1);
                    }
                } else
                    _iScan.onDecode(data);
